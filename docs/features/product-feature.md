    ...
    <div className="flex flex-col gap-4 md:gap-6">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList>
          ...
          <TabsTrigger value="iad">自營廣告 (IAD)</TabsTrigger>
          <TabsTrigger value="ext">外媒 (EXT)</TabsTrigger>
          <TabsTrigger value="svc">服務 (SVC)</TabsTrigger>
          <TabsTrigger value="pkg">專案 (PKG)</TabsTrigger>
        </TabsList>
...
export function ProductsTable({ filters }: ProductsTableProps) {
  ...
  const { data, isLoading } = trpc.product.products.list.useQuery({
    mainCode: filters?.mainCode as any,
  ...
  const publishMutation = trpc.product.products.publish.useMutation({
    onSuccess: () => utils.product.products.list.invalidate(),
  });
  const deactivateMutation = trpc.product.products.deactivate.useMutation({
    onSuccess: () => utils.product.products.list.invalidate(),
  });
  const deleteMutation = trpc.product.products.delete.useMutation({
    onSuccess: () => utils.product.products.list.invalidate(),
  });

  const handleClone = (productCode: string) => {
          ...
          {data?.data.map((product) => (
            <TableRow key={product.id}>
              ...
              <TableCell className="text-xs">
                {product.startDate ? format(new Date(product.startDate), 'yyyy/MM/dd') : '-'}
              </TableCell>
              <TableCell className="text-xs">
                {product.endDate ? format(new Date(product.endDate), 'yyyy/MM/dd') : '-'}
              </TableCell>
              <TableCell>
                <div className="flex gap-1">
                  ...
                  <Button
                    variant="ghost"
                    size="icon"
                    onClick={() => handleClone(product.productCode!)}
                  >
                  ...
                  {product.status === 'draft' && (
                    <Button
                      variant="ghost"
                      size="icon"
                      onClick={() => publishMutation.mutate({ id: product.id, publishedBy: 'current_user' })}
                    >
                  ...
                  {product.status === 'published' && (
                    <Button
                      variant="ghost"
                      size="icon"
                      onClick={() => deactivateMutation.mutate({ id: product.id, deactivatedBy: 'current_user' })}
                    >
